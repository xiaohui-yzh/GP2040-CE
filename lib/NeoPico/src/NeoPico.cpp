...
void NeoPico::PutPixel(uint32_t pixelData) {
  switch (format) {
    case LED_FORMAT_GRB:
    case LED_FORMAT_RGB:
      pio_sm_put_blocking(this->pio, this->sm, pixelData << 8u);
      break;
    case LED_FORMAT_GRBW:
    case LED_FORMAT_RGBW:
      pio_sm_put_blocking(this->pio, this->sm, pixelData);
      break;
  }
}

NeoPico::NeoPico(PIO _pio, int _sm, int ledPin, int numPixels, LEDFormat format) : format(format), numPixels(numPixels) {
  pio = _pio;
  sm = _sm;
